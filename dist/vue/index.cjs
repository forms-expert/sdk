...
__export(vue_exports, {
  FORMS_SDK_KEY: () => FORMS_SDK_KEY,
  createFormsPlugin: () => createFormsPlugin,
  useForm: () => useForm
});
...
function useForm(options) {
  ...
  const config = (0, import_vue.ref)(null);
  const isLoading = (0, import_vue.ref)(false);
  const isInitializing = (0, import_vue.ref)(false);
  const isSubmitted = (0, import_vue.ref)(false);
  const errors = (0, import_vue.ref)({});
  const values = (0, import_vue.ref)({});
  const activeTheme = (0, import_vue.ref)(options.theme);
  ...
  const requiresCaptcha = (0, import_vue.computed)(() => config.value?.settings?.captcha?.enabled ?? false);
  const captchaProvider = (0, import_vue.computed)(() => config.value?.settings?.captcha?.provider);
  const captchaSiteKey = (0, import_vue.computed)(() => config.value?.settings?.captcha?.siteKey);
  const honeypotEnabled = (0, import_vue.computed)(() => config.value?.settings?.honeypot ?? false);
  const availableThemes = (0, import_vue.computed)(() => config.value?.availableThemes || []);
